...
def build_affect_guidance(profile: dict) -> str:
    ...
    if profile.get("hostile_recent", 0) >= 2 and "apologetic" in profile.get("recent_tones", []):
        lines.append("- Пользователь извинился после грубости, но не забывай конфликт мгновенно. Смягчайся постепенно, без резкого \"ничего страшного\".")
